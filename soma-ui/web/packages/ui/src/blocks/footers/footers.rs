use crate::components::inputs::button::{Button, ButtonSize, ButtonVariant};
use crate::components::layout::separator::Separator;
use leptos::prelude::*;

#[component]
pub fn FooterBlock() -> impl IntoView {
    view! {
        <footer class="bg-card border-t border-border">
            <div class="max-w-6xl mx-auto px-6 py-12">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    // Brand column
                    <div class="space-y-3 lg:col-span-1">
                        <span class="font-heading font-bold text-foreground">"Soma UI"</span>
                        <p class="text-sm text-muted-foreground">"Composable Leptos components with a Palantir-inspired design system."</p>
                    </div>

                    // Product links
                    <div class="space-y-3">
                        <p class="text-sm font-semibold text-foreground">"Product"</p>
                        <ul class="space-y-2">
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"Features"</a></li>
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"Pricing"</a></li>
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"Changelog"</a></li>
                        </ul>
                    </div>

                    // Docs links
                    <div class="space-y-3">
                        <p class="text-sm font-semibold text-foreground">"Docs"</p>
                        <ul class="space-y-2">
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"Getting Started"</a></li>
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"Components"</a></li>
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"API Reference"</a></li>
                        </ul>
                    </div>

                    // Company links
                    <div class="space-y-3">
                        <p class="text-sm font-semibold text-foreground">"Company"</p>
                        <ul class="space-y-2">
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"About"</a></li>
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"Blog"</a></li>
                            <li><a href="#" class="text-sm text-muted-foreground hover:text-foreground transition-colors">"Contact"</a></li>
                        </ul>
                    </div>
                </div>

                <div class="mt-10">
                    <Separator />
                    <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <p class="text-xs text-muted-foreground">"© 2025 Soma UI. All rights reserved."</p>
                        <div class="flex items-center gap-1">
                            <Button variant=ButtonVariant::Ghost size=ButtonSize::Icon aria_label="GitHub".to_string()>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                    <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8"/>
                                </svg>
                            </Button>
                            <Button variant=ButtonVariant::Ghost size=ButtonSize::Icon aria_label="Twitter".to_string()>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                    <path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.07-4.425 5.07H.316l5.733-6.57L0 .75h5.063l3.495 4.633L12.601.75Zm-.86 13.028h1.36L4.323 2.145H2.865z"/>
                                </svg>
                            </Button>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    }
}

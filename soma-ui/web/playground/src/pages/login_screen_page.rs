...
use leptos::prelude::*;
use soma_ui::LoginScreen;

#[component]
pub fn LoginScreenPage() -> impl IntoView {
    let locale = use_context::<RwSignal<Locale>>().expect("locale context");
    let s = move || strings(locale.get());

    view! {
        <div class="max-w-5xl">
            <h1 class="font-heading text-2xl font-bold text-foreground">{move || s().screen_login_title}</h1>
            <p class="text-sm text-muted-foreground mt-1 mb-4">{move || s().screen_login_subtitle}</p>
            ...
            <div class="rounded-md border border-border overflow-hidden">
                ...
                <div class="h-9 bg-muted flex items-center gap-1.5 px-3 border-b border-border">
                    ...
                    <div class="ms-3 flex-1 max-w-xs h-5 rounded bg-background/60 border border-border flex items-center px-2">
                        <span class="text-xs text-muted-foreground font-mono">"foundry.io/login"</span>
                    </div>
                </div>
